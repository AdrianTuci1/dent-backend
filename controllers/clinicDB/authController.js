...
const { ClinicUser } = require('../../models/clinicDB/clinicUser');
    ...
    try {
      ...
      const token = jwt.sign(
        {
          userId: user.id,
          role: user.role,
          clinicId: user.role === 'clinic' ? user.id : user.clinicId, // Include clinicId for clinics and medics
        },
      ...
      if (user.role === 'clinic') {
        ...
        const subaccounts = await ClinicUser.findAll({
          where: { subaccount_of: user.id, role: 'medic' },
